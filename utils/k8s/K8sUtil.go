	...
	if resourceReference["name"] != "" {
		...
		case ReplicaSetKind:
			...
			if strings.Contains(resName, "-") {
				...
				allowed := validateCallback(namespace, groupName, DeploymentKind, deploymentName)
				if allowed {
					return true
				}
		...
		case DeploymentKind, K8sClusterResourceCronJobKind, StatefulSetKind, DaemonSetKind, K8sClusterResourceRolloutKind, K8sClusterResourceReplicationControllerKind:
			allowed := validateCallback(namespace, groupName, resKind, resName)
			if allowed {
				return true
			}
